...
/**
 * 
 * An extended class to provide methods for the game board
 * It also stores the game board, current turn and winner. 
 */
		...
		for (int turn = RED; turn <= YELLOW; turn++) {
			...
			for (int row=0;row<NUM_ROWS-4;row++) {
				...
				for (int col=0;col<NUM_COLS-4;col++) {
					...
					if (getCounterAt(col,row) == turn && getCounterAt(col+1,row+1) == turn && getCounterAt(col+2,row+2) == turn && getCounterAt(col+3,row+3) ==turn) {
						winner = turn;
						return true;
					}
				}
