...
public class Connect4GameStateTest {
    ...
    private void checkMove(String msg, Connect4GameState gs, int col, int row, int counter) {
        ...
        try {
            gs.move(col);
            assertEquals(msg + "move made incorrectly - checking (" + col + "," + row + ")",
                    counter, gs.getCounterAt(col, row));
            assertEquals(msg + "next turn counter not changed correctly",
                    counter == RED ? YELLOW : RED, gs.whoseTurn());
